...
#   SANDBOX_MODE=1         # Skip Docker checks
#   FULL_VERIFY=1          # Force full verification every iteration
#   VALIDATE_INTERVAL=N    # Opus validation every N iterations (default: 5)
...
SANDBOX_MODE="${SANDBOX_MODE:-0}"     # Skip Docker checks
...
for arg in "$@"; do
  case "$arg" in
    ...
    -h|--help)
      echo -e "${BOLD}Agentic Workflow — Agent Loop${RESET}"
      echo ""
      ...
      echo "  2. Selects next open requirement (P0 > P1 > P2)"
      ...
      echo "  Every 5 iterations (dynamic: 3 after issues) Opus reviews:"
      echo "  - Whether done REQs actually work (Docker + Playwright)"
      echo "  - Whether the agent circumvented rules (log analysis)"
      ...
      echo "  + Cost tracking per iteration and cumulative"
      echo ""
      echo -e "${BOLD}Auto-stop conditions:${RESET}"
      echo "  - All requirements done"
      echo "  - Max iterations reached"
      echo "  - Iteration timeout (default: 30 min, ITER_TIMEOUT=N)"
      echo "  - 2x low-activity (≤5 tools, not blocked) consecutively"
      ...
      echo "  SANDBOX_MODE=1       Skip Docker checks"
      echo "  VALIDATE_INTERVAL=N  Opus validation every N iterations (default: 5)"
      echo "  VALIDATOR_TIMEOUT=N  Timeout for validation in seconds (default: 2400)"
      echo ""
      echo -e "${BOLD}Files:${RESET}"
      echo "  AGENT.md                Agent iteration prompt"
      echo "  VALIDATOR.md            Opus validation prompt (optional)"
      echo "  .agent/logs/            Stream-JSON logs per iteration"
      echo "  PRD.md                  Requirements with status"
      ...
      echo "  .agent/architecture.md  Architecture decisions (append-only)"
      echo "  .agent/learnings.md     Persistent learnings (append-only)"
      echo "  .agent/iterations.jsonl Iteration log (append)"
      echo ""
      echo -e "${BOLD}Examples:${RESET}"
      echo "  ./loop.sh          # Run until all REQs done"
...
build_agent_prompt() {
  ...
  if [ -n "$req_block" ]; then
    ...
    echo "---"
    echo ""
    echo "## Prepared Context (injected by loop.sh)"
    echo ""
    echo "Next open requirement (dependencies checked):"
    echo ""
    echo "$req_block"
    echo ""
    ...
    echo "Read .agent/context.md and PRD.md yourself — this pre-selection is a hint, not a guarantee."
  fi
}
...
build_validator_prompt() {
  ...
  echo "---"
  echo ""
  echo "## Injected Context (by loop.sh)"
  echo ""
  ...
  echo ""
  echo "### Full logs if needed"
  echo ""
  echo "Full stream-json logs are in \`$LOG_DIR/\`. Use the Read tool for details."
  echo ""
  echo "### Current Status"
  echo ""
...
kill_dev_servers() {
  ...
  # Customize the ports for your project
  local pids
  pids=$(lsof -ti:3572,5173,5174,4321,3000 2>/dev/null || true)
  if [ -n "$pids" ]; then
    echo -e "  ${DIM}Cleaning up leftover dev servers${RESET}"
...
echo -e "${BOLD}Agentic Workflow — Agent Loop${RESET}  ${DIM}model=${MODEL}  max=$([ "$MAX_ITERATIONS" -eq 0 ] && echo '∞' || echo "$MAX_ITERATIONS")  timeout=$(format_duration "$ITER_TIMEOUT")${RESET}${SANDBOX_LABEL}"
...
while :; do
  ...
  # Backup status.json before iteration
